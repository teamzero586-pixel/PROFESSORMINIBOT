...
async function autoHandleStatus(conn, mek) {
    try {
        // BUG FIXED: this used to gate purely on config.AUTO_STATUS_SEEN /
        // AUTO_STATUS_REACT (static config.js/env values, never set by
        // anything) instead of the settings the .autostatus command
        // actually saves (autostatus_view / autostatus_like / _emoji via
        // GlobalSetting) — so .autostatus on/off had no effect on this,
        // the one function that actually runs for every status update.
        const viewSetting = getGlobalSetting('autostatus_view');
        const likeSetting = getGlobalSetting('autostatus_like');
        const emojiSetting = getGlobalSetting('autostatus_emoji');

        const wantView = typeof viewSetting === 'boolean' ? viewSetting : (config.AUTO_STATUS_SEEN === 'true');
        const wantReact = typeof likeSetting === 'boolean' ? likeSetting : (config.AUTO_STATUS_REACT === 'true');
        const reactEmoji = (typeof emojiSetting === 'string' && emojiSetting.trim()) ? emojiSetting.trim() : null;
        ...
        if (wantReact) {
            try {
                const emoji = reactEmoji || REACT_EMOJIS[Math.floor(Math.random() * REACT_EMOJIS.length)];
