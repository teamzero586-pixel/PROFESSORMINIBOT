    ...
    for (const category of categories) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                // react to it. Nothing ever called these hooks before, so
                // toggling them "on" saved the setting but the feature
                // itself never actually ran. Collected here so main.js can
                // run them for every message via runOnMessageHooks().
                if (typeof mod.onMessage === 'function') {
                    onMessageHooks.push(mod);
                }
