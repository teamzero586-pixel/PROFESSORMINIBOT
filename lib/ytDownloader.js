...
const PROVIDERS = [
  {
    ...
    async fetch(url) {
      ...
      const mp4Url = result?.downloadUrl || result?.download_url || result?.url || result?.mp4;
      if (!mp4Url) throw new Error('arslan-apis-v2: no download url in response — ' + JSON.stringify(data).slice(0, 200));
      return {
        mp4Url,
        title: result?.title,
  ...
  },
  {
    ...
    async fetch(url) {
      ...
      const { data } = await fetchWithRetry(apiUrl, 2, 20000);
      if (!data?.status || !data?.mp4) throw new Error('tioo: no mp4 in response — ' + JSON.stringify(data).slice(0, 200));
...
async function downloadYoutube(url, opts = {}) {
  ...
  try {
    ...
    if (durationSec > 0 && durationSec <= maxDurationSec) {
      ...
      ytdlError = `video is ${durationSec}s, over the ${maxDurationSec}s limit`;
    }
  } catch (e) {
    ytdlError = e.message;
  ...
  throw new Error(`All download sources failed (ytdl-core: ${ytdlError || 'skipped'} | ${errors.join(' | ')}). YouTube may be blocking this video, or the video is unavailable — try again in a bit.`);
}
...
const AUDIO_PROVIDERS = [
  {
    name: 'arslan-apis-v2',
    async fetch(url) {
      const apiUrl = `https://arslan-apis-v2.vercel.app/download/ytmp3?url=${encodeURIComponent(url)}`;
      const { data } = await fetchWithRetry(apiUrl, 2, 20000);
      const result = data?.result || data;
      const mp3Url = result?.downloadUrl || result?.download_url || result?.url || result?.mp3;
      if (!mp3Url) throw new Error('arslan-apis-v2: no download url in response — ' + JSON.stringify(data).slice(0, 200));
      return {
        mp3Url,
        title: result?.title,
        thumbnail: result?.thumbnail || result?.image,
...
async function downloadYoutubeAudio(url, toAudio) {
  ...
  try {
    ...
    throw new Error(`Audio sources failed (${errors.join(' | ')}); video fallback also failed: ${videoErr.message}`);
  }
}
